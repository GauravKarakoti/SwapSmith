...
import { Zap, ArrowRight, PanelLeft, HelpCircle, LogOut, LineChart } from 'lucide-react';
...
export default function Navbar({ isSidebarOpen, onToggleSidebar }: NavbarProps) {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 border-b border-zinc-800 backdrop-blur-xl ${
      isTerminal ? 'h-16 bg-zinc-900/30' : 'h-16 sm:h-20 bg-[#050505]/80'
    }`}>
      <div className={`${isTerminal ? 'px-4' : 'max-w-7xl mx-auto px-4 sm:px-6'} h-full flex justify-between items-center`}>
