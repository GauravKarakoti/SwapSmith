...
// --- REPUTATION & TRUST SCORE FUNCTIONS ---
...
export async function calculateReputationMetrics(userId: string): Promise<ReputationMetrics> {
  if (!db) {
    console.warn('Database not configured');
    return {
      totalSwaps: 0,
  ...
  try {
    // Fetch all swaps for the user
    const userSwaps = await db.select().from(swapHistory)
      .where(eq(swapHistory.userId, userId));
    ...
    const successfulSwaps = userSwaps.filter(s => s.status === 'completed' || s.status === 'settled').length;
    const failedSwaps = userSwaps.filter(s => s.status === 'failed' || s.status === 'cancelled').length;
    const pendingSwaps = userSwaps.filter(s => s.status === 'pending' || s.status === 'processing').length;
    
    const successRate = totalSwaps > 0 ? (successfulSwaps / totalSwaps) * 100 : 0;

    // Calculate total volume (sum of settlement amounts)
    const totalVolumeSwapped = userSwaps.reduce((sum, swap) => {
    ...
    if (lastSwapDate) {
      const daysSinceLastSwap = (Date.now() - lastSwapDate.getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceLastSwap <= 7) {
        recentActivity = 'active';
      } else if (daysSinceLastSwap <= 30) {
        recentActivity = 'moderate';
      }
    }

    // Calculate trust score (0-100 scale)
    // Based on: success rate (60%), volume (20%), swap count (10%), recency (10%)
    const volumeScore = Math.min((totalVolumeSwapped / 10000) * 100, 100); // Normalize to $10k baseline
    const countScore = Math.min((totalSwaps / 50) * 100, 100); // Normalize to 50 swaps baseline
    const recencyScore = recentActivity === 'active' ? 100 : recentActivity === 'moderate' ? 50 : 0;
    ...
    return {
      ...
      avgSwapValue: Math.round(avgSwapValue * 100) / 100,
      totalVolumeSwapped: Math.round(totalVolumeSwapped * 100) / 100,
      lastSwapDate,
      recentActivity,
    };
  } catch (error) {
...
export async function calculateReputationMetricsByWallet(walletAddress: string): Promise<ReputationMetrics> {
  if (!db) {
    console.warn('Database not configured');
    return {
      totalSwaps: 0,
  ...
  try {
    // Fetch all swaps for the wallet
    const walletSwaps = await db.select().from(swapHistory)
      .where(eq(swapHistory.walletAddress, walletAddress));
    ...
    // Calculate metrics (same logic as userId version)
    const totalSwaps = walletSwaps.length;
    const successfulSwaps = walletSwaps.filter(s => s.status === 'completed' || s.status === 'settled').length;
    const failedSwaps = walletSwaps.filter(s => s.status === 'failed' || s.status === 'cancelled').length;
    const pendingSwaps = walletSwaps.filter(s => s.status === 'pending' || s.status === 'processing').length;
    
    const successRate = totalSwaps > 0 ? (successfulSwaps / totalSwaps) * 100 : 0;

    const totalVolumeSwapped = walletSwaps.reduce((sum, swap) => {
      const amount = typeof swap.settleAmount === 'string' 
        ? parseFloat(swap.settleAmount) 
        : swap.settleAmount;
      return sum + (isNaN(amount) ? 0 : amount);
    ...
    const volumeScore = Math.min((totalVolumeSwapped / 10000) * 100, 100);
    const countScore = Math.min((totalSwaps / 50) * 100, 100);
    const recencyScore = recentActivity === 'active' ? 100 : recentActivity === 'moderate' ? 50 : 0;
    ...
    console.error('Error calculating reputation metrics by wallet:', error);
    return {
      totalSwaps: 0,
      successfulSwaps: 0,
      failedSwaps: 0,
