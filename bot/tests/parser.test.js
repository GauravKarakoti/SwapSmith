...
const groq_client_1 = require("../src/services/groq-client");
// Mock groq-sdk
jest.mock('groq-sdk', () => {
    return {
        __esModule: true,
...
describe('parseUserCommand Regex Logic', () => {
    ...
    it('parses "Swap everything to BTC"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap everything to BTC');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('all');
        expect(res.toAsset).toBe('BTC');
    });
    it('parses "Swap all ETH to USDT"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap all ETH to USDT');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('all');
    ...
    it('parses "Swap max MATIC"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap max MATIC');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('all');
    ...
    it('parses "Swap 50% ETH to USDT"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap 50% ETH to USDT');
        expect(res.intent).toBe('swap');
    ...
    it('parses "Swap 25 percent SOL to ETH"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap 25 percent SOL to ETH');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('percentage');
    ...
    it('parses "Swap half BTC to SOL"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap half BTC to SOL');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('percentage');
        expect(res.amount).toBe(50);
    ...
    it('parses "Swap everything except 10 MATIC"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Swap everything except 10 MATIC');
        expect(res.intent).toBe('swap');
        expect(res.amountType).toBe('all');
        // @ts-ignore
    ...
    it('parses "Convert 100 USDT into BTC"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Convert 100 USDT into BTC');
        expect(res.intent).toBe('swap');
    ...
    it('parses "Send max BTC"', async () => {
        const res = await (0, groq_client_1.parseUserCommand)('Send max BTC');
