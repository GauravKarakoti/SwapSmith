...
async function handleTextMessage(ctx: any, text: string, inputType: 'text' | 'voice' = 'text') {
    ...
    // Note: You'll need to implement parseUserCommand or import it
    // For now, adding a placeholder type to avoid errors
    const parsed: any = await parseUserCommand(text, history, inputType);
...
// --- Placeholder for parseUserCommand ---
// You need to implement this or import from your services
async function parseUserCommand(text: string, history: any[], inputType: 'text' | 'voice'): Promise<any> {
