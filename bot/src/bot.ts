...
async function handleTextMessage(ctx: any, text: string) {
  ...
  if (state?.parsedCommand && !state.parsedCommand.settleAddress) {
      ...
      return ctx.reply(
        `âœ… Address resolved: \`${resolved.address}\`\n\nProceed?`,
        {
          ...
          ...Markup.inlineKeyboard([
            ...
            Markup.button.callback('âŒ Cancel', 'cancel_swap'),
          ]),
        }
      );
    }
    ...
    if (isNamingService(text)) {
      return ctx.reply(`âŒ Could not resolve domain. Try a raw address.`);
    }

    return ctx.reply(`âŒ Invalid address. Try again or /clear.`);
  }
